...
@Component({
  selector: 'app-inventory-supervisor',
  templateUrl: './inventory-supervisor.component.html',
  styleUrls: ['./inventory-supervisor.component.css']
})
export class InventorySupervisorComponent implements OnInit {
  ...
  bindRowData() {
    this._stockAllotmentService.getSupervisorSubmitStockRow().subscribe((result: any) => {
      if (result != null) {
  ...
  submitRow() {
    if (this.selectedRow !== undefined) {
      ...
      this._stockAllotmentService.submitRowBySupervisor(submitRequest).subscribe((result: any) => {
        if (result) {
          this.bindRowData();
          this.selectedRow = undefined;
          this.allInventory = [];
